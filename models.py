...
class TripShare(db.Model):
    ...
    def to_dict(self):
        return {
            'id': self.id, 'trip_id': self.trip_id, 'user_id': self.user_id,
...
class NearbyPOI(db.Model):
    ...
    def to_dict(self):
        return {
            ...
            'subcategory': self.subcategory, 'city': self.city, 'province': self.province,
            'address': self.address, 'latitude': self.latitude, 'longitude': self.longitude,
            'rating': self.rating, 'price_level': self.price_level, 'phone': self.phone,
            'opening_hours': self.opening_hours, 'description': self.description,
            'tags': json.loads(self.tags) if self.tags else [],
            'images': json.loads(self.images) if self.images else [],
            'cover_image': self.cover_image, 'business_status': self.business_status,
            'distance': self.distance, 'destination_id': self.destination_id,
            'created_at': self.created_at.strftime('%Y-%m-%d %H:%M') if self.created_at else None
        }
...
class UserLike(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'user_id': self.user_id, 'destination_id': self.destination_id,
                'created_at': self.created_at.strftime('%Y-%m-%d %H:%M') if self.created_at else None}
...
class UserFollow(db.Model):
    ...
    def to_dict(self):
        return {'id': self.id, 'follower_id': self.follower_id, 'following_id': self.following_id,
                'created_at': self.created_at.strftime('%Y-%m-%d %H:%M') if self.created_at else None}
